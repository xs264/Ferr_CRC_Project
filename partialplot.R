extract_partial<-function(i,result,time,npts){
  ...
  output <- as.data.frame(cbind(gene.name=rep(gene_list[i],npts), 
                                ...
                                # survprob_l=unname(sapply(1:npts, function(j) quantile(gene_i.year$survprob[seq(j,  npts*B, by = npts)],probs=0.0npts))),
                                # survprob_u=unname(sapply(1:npts, function(j) quantile(gene_i.year$survprob[seq(j,  npts*B, by = npts)],probs=0.975))),
